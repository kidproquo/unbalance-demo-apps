...
def extract_minimal_features(window_data):
    ...
    Args:
        window_data: Array of shape (n_samples, 4) with columns [Measured_RPM, Vibration_1, Vibration_2, Vibration_3]
...
def process_from_redis(model, output_dir, redis_config,
                      ...
                      consumer_name='rfc', log_interval=10, log_timing=False, data=None):
    """
        ...
        while True:
            ...
            if detection_ratio > 0.5:
                ...
                detection_event = {
                    ...
                    'confidence': np.max(predictions).item() if hasattr(np.max(predictions), 'item') else float(np.max(predictions)),
                    'figure_file': filename,
                    'approach': 'rfc'
                }
            ...
            if log_timing:
                print(f"[RFC] Window {window_idx}: read={read_time*1000:.1f}ms, parse={parse_time*1000:.1f}ms, "
